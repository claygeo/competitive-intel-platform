...
A production data engineering system that automates competitive pricing analysis across 1,000+ dispensary locations in 15 states. Built to replace a manual process that took my team 2+ weeks per cycle, now runs on autopilot around the clock on 2-6 hour cycles with real-time Telegram alerting.
...
- Scrapes 175,000+ products across 1,000+ store locations in 15 states
...
- Refreshes everything on automated 2-6 hour cycles with Telegram status alerts
...
┌──────────────────────────────────────────────────────────────┐
│                     SCHEDULER (Cron)                         │
│      ~100 jobs staggered 30s apart on 2-6 hour cycles        │
└─────────────────────────┬────────────────────────────────────┘
                          │
                          ▼
┌──────────────────────────────────────────────────────────────┐
...
│  │                  │  │                  │  │            │  │
│  │  50+ variations  │  │  2,200+ brands   │  │ Tax norm,  │  │
...
**Brand problem:** 2,200+ brands with inconsistent naming across 1,000+ store locations. The brand alias engine resolves all variations to canonical brand names using exact matching, case-insensitive matching, and configurable alias tables.
...
Around 100 scraper jobs run across 3 concurrent workers. BullMQ job IDs are deterministic, so if cron fires while a manual trigger is already running, the duplicate collapses into the existing job. No wasted compute, no race conditions.

An in-memory cycle tracker collects results from every job in the cycle and fires a consolidated Telegram summary when the last one completes (or after a 4-hour timeout). One message per cycle instead of a hundred separate notifications.
...
| Scheduling | Cron: 2-6 hour cycles (products), twice daily (store hours), weekly (brand snapshots) |
...
- **State Scoping:** Cookie-based state selector (15 states). Every query filters server-side.
...
| Metric | Value |
|--------|-------|
| Products tracked | 175,000+ |
| Historical price points | 13M+ |
| Store locations | 1,000+ |
| States covered | 15 |
| API platforms reversed | 3 (REST, GraphQL, Algolia) |
| Scrape cycle | Every 2-6 hours + on-demand |
